...
//move the text to specific places of the screen
...
impl App for BlackjackAid {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        let visuals = egui::Visuals {
            window_fill: egui::Color32::from_rgb(10, 10, 40), // dark gray background
            ..egui::Visuals::dark() // start from dark theme and customize
        };

        ctx.set_visuals(visuals);
        
        egui::CentralPanel::default().frame(egui::Frame::default().fill(egui::Color32::from_rgb(0,150,0))).show(ctx, |ui| {
            ...
            ui.label(egui::RichText::new(format!("{}", self.recorded_cards)).color(egui::Color32::BLUE));
        });
    }
}
